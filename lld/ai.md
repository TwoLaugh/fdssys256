...
>
> **Admin authorisation (ai-10, BUILT — enforced; consolidated project-wide).** `@PreAuthorize("hasRole('ADMIN')")` on `AdminAiController` is **inert** in v1 (the project does not enable `@EnableMethodSecurity` and the flat user model attaches only `ROLE_USER` — there is no `ROLE_ADMIN`). Rather than enabling method-security project-wide (which would simultaneously 403 every other module's admin endpoint), the admin AI surface is gated **imperatively**. As of the project-wide admin-auth consolidation, the AI module's original `AiAdminGuard` + `mealprep.ai.admin.user-ids` key have been **migrated to the shared `AdminAccessGuard` (in `auth.api`) backed by the single allowlist `mealprep.admin.user-ids`** — the one mechanism every module's admin controller now uses. Still **fail-closed** (empty default ⇒ no non-admin reaches it): anonymous → 401, authenticated-non-admin → 403. When project-wide method-security lands, the `@PreAuthorize` annotations activate and this guard can retire.
>
...
> **v1 prompt-assembly state (finding `ai-1`, deferred to v1.5).** Steps 2–3 above describe template-rendering-at-dispatch (`PromptTemplateService.loadCurrent` → `render`) as the prompt-assembly path, and the `AiTask` SPI carries `getSystemPrompt()` / `getUserPromptRef()`. **In the shipped v1 this is *not* wired into dispatch.** Each calling-module `AiTask` impl hand-assembles its own prompt and ships the fully-rendered text under the conventional `"prompt"` variable (`AiTask.variables().get("prompt")`); `AnthropicClient` sends that string verbatim as the single user message and sends **no system prompt** at dispatch. This hand-assembled-prompt approach is the **accepted v1 state**. Wiring `PromptTemplateService` rendering into dispatch *and* threading `getSystemPrompt()` into the Anthropic `system` param is **deferred to v1.5**, gated on a prompt-eval harness: it is a cross-module migration of all ~8–10 `AiTask` impls, is structurally invisible to the test suite (the `@Profile({"test","e2e"})` `TestAiService` stubs dispatch and never reaches `AnthropicClient`), and introducing system prompts would shift AI outputs with no integration eval to catch a regression. Until then, treat the prose in Flow 1 steps 2–3 and the `getSystemPrompt()` / `getUserPromptRef()` SPI members as the v1.5 target, not the v1 contract.
...
Circuit breaker: Resilience4j circuit breaker named `ai-${taskType}`, opening after 5 windowed failures and staying open for 5 minutes (HLD); open breaker → `AiCircuitOpenException` (503). **As built (ticket `ai-2`) it is applied *programmatically*** — `AnthropicClient` resolves the breaker from a `CircuitBreakerRegistry` keyed `ai-${taskType}` and guards each `call(...)` with `tryAcquirePermission()` / `onSuccess` / `onError` — **not** via the `@CircuitBreaker` AOP annotation. Programmatic decoration sidesteps the Spring-AOP self-invocation trap (the per-attempt retry loop lives inside the same method, so annotation advice would never see the individual wire calls) and keeps the resilience path exercisable by the WireMock unit tests that `new`-construct the client. Only transient failures (`AiUnavailableException` incl. its `AiRateLimitException` subtype, and `ResourceAccessException`) count toward opening the breaker; fatal caller-bug 4xx (`AiInvalidRequestException`) are ignored by it. The classified retry loop (5xx/IO → `TIMEOUT` short backoff; 429 → `RATE_LIMIT` longer backoff; 4xx → fatal, no retry) runs *inside* the breaker permission.
