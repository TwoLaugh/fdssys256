...
3. **AI parse step** — _wired (nutrition-01k)._ Calls `IngredientParseTask` (cheap tier) via `AiService` to produce a clean `usdaSearchTerm` plus the structured parse `{ingredient, quantity, unit, gramsEstimate, usdaSearchTerm, isCooked}`; the cleaned term drives the search. **Graceful degrade:** if the AI call fails (circuit-open, budget, parse error) or returns no usable term, the pipeline falls back to the normalised term verbatim — the original 01d behaviour. Recipe-side calls already carry structured form. The cache-check (step 2) runs FIRST and is keyed on the normalised term, so each unique term is parsed at most once, then cached — repeats incur no AI cost.
...
5. **Match selection** — _wired (nutrition-01k)._ `IngredientMatchTask` (cheap tier, via `AiService`) re-ranks the bounded candidate list (≤ 8 hits, cost-capped) and picks the best by index, returning `{chosenIndex, confidence, reason}`; `chosenIndex = -1` means "no good match". On a genuine AI pick the **0.85 source-score cap lifts** and the model's own confidence is persisted. **Graceful degrade (safety net preserved):** if the AI call fails, the breaker is open, the model declines (`-1`), or the chosen index is out of range, the pipeline takes the **first** (highest source-score) USDA hit, else the first OFF hit, and **caps the source-derived confidence at 0.85** — so a degraded first-hit mapping lands at ≤ 0.85 and a low score still flips `needsReview`. Both clients empty → `UnmappedIngredientDto(name, "no source matches", 0)`; recipe `nutritionStatus` becomes `partial`.
