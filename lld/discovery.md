...
| Within-job source/candidate iteration | **v1 default remains single-threaded per job.** With the default flag off, the search phase iterates requested sources sequentially and the fetch phase iterates surviving candidates sequentially on the single runner thread; the `discoveryRunnerExecutor` pool parallelises *across* jobs (one `run()` per job), not *within* a job. This keeps the order-dependent invariants simple and race-free: the quota stop (`recipesIngested == requestedCount`), the per-candidate cancellation check, and the `sourcesSucceeded` / `sourcesFailed` / `recipesIngested` accumulators are all mutated from one thread with no shared-state hazard. **Cross-source search parallelism is now available behind `mealprep.discovery.parallel-sources` (default `false`).** When ON, each source's `search(...)` is fanned out onto a *separate* bounded pool (`discoverySourceFanoutExecutor`, distinct from `discoveryRunnerExecutor` to avoid pool-exhaustion deadlock) via `CompletableFuture` + `allOf` with a per-source timeout (`parallel-source-timeout`, default 30s). Thread-safety is by *isolation, not locking*: each source runs `searchSingleSource(...)` and returns an **immutable** per-source result touching only already-thread-safe collaborators (`SourceRateLimiterRegistry`, `SourceRegistry` per-key `@Transactional` writes, the append-only scrape-log write); the runner thread then merges those results **single-threaded, in deterministic request order** after join — so `merged` / `sourcesFailed` have exactly one writer and the candidate list is byte-for-byte order-stable regardless of completion order. The quota gate and per-candidate cancellation are **unchanged** — they live in the still-sequential fetch phase, so quota/cancellation semantics are identical to the default. Partial failure stays isolated per future (one source throwing never sinks the batch); a source that times out is cancelled and recorded in `sourcesFailed`, mirroring the sequential failure outcome. The fetch phase stays sequential by design (it is per-*candidate*, not per-source, and the quota/cancellation invariants are inherently order-dependent). |
