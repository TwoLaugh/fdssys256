...
- **URL import AI extraction prompt.** The deterministic `HtmlImportParser` covers the v1 data path. The AI extractor (mid-tier model, tool-use structured output per HLD) replaces the parser when its prompt and schema are specified — the catalogue endpoint is unchanged.
- **Per-section AI generation of new recipes.** Enters via `RecipeWriteApi.saveAdaptedVersion` after the pipeline produces a body.
- **Recipe images.** Imported recipes often include image URLs and manual entries can upload files; image storage is deferred — no `image_url` column in v1, revisit alongside the UI phase.
