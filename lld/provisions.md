...
Cook-event, meal-consumption, and standalone-consumption flows DO have an operator/test REST surface in the shipped build (`CookEventController`: `POST /cook-event`, `POST /meal-consumption`, `POST /standalone-consumption`); grocery import is exposed via `GroceryImportController` (`POST /grocery-import`). **Reconciliation note (2026-05):** the original LLD kept these in-process only, matching the pilot's "in-process methods off the HTTP boundary" pattern. In the shipped system the planner publishes no `MealCookedEvent` and the grocery module performs its inventory add via a direct, idempotent `ProvisionUpdateService.applyGroceryOrder` call at reconcile time (not an event). The REST seam was therefore added so these flows are exercisable end-to-end by operators and integration tests; `userId` is server-resolved (the caller can only act on their own pantry). The underlying `ProvisionUpdateService` methods remain the in-process entry points; the controllers are thin adapters over them.
...
`ItemNearingExpiryEvent` and the daily expiry sweep that generates it are **owned by the notification module** (notification/01b), not provisions — see Flow 8. The `ItemNearingExpiryEvent` record above documents the contract the notification scanner emits; provisions only supplies the read helpers it scans (`getExpiringInventory`, `findAllExpiringBefore`). Provisions publishes none of this itself.
...
### Consumed — *none (reconciled 2026-05)*

The originally-designed event-listener seams (`MealCookedEvent`, `MealConsumedEvent`, `GroceryOrderConfirmedEvent`) are **not** built as provisions listeners in v1. The shipped wiring is direct, in-process:

- **Cook / meal-consumption** are invoked directly via `ProvisionUpdateService.applyCookEvent` / `applyMealConsumption` (the `CookEventController` REST seam fronts them). The planner publishes no `MealCookedEvent`/`MealConsumedEvent` — those records do not exist in the codebase.
- **Grocery import** is invoked directly via `ProvisionUpdateService.applyGroceryOrder`, called by the grocery module's `OrderReconciler` at reconcile time (idempotent on `(userId, source, orderRef)`). `GroceryOrderConfirmedEvent` IS published by the grocery module but as a lifecycle/notification signal only — provisions does not consume it (a confirm-time add would double-add against the reconcile-time path).
...
The caller (operator/test via `POST /cook-event`, or an in-process caller) invokes `applyCookEvent` with the recipe ingredient list at cook time. `@Transactional(noRollbackFor = InventoryUnderflowException.class)`. *(v1: this is a direct call, not a `MealCookedEvent` listener — see §Events Consumed.)*

0. **Gate.** When the user has `pantry_tracking_enabled = false` (read via the `LifestyleConfigQueryService` SPI) the whole flow is a no-op returning an empty result (LLD §Pantry tracking disable).
...
4. **Batch cook (built).** If `isBatchCook`, `BatchCookSplitter` creates a fridge row (`storageLocation = FRIDGE`, `quantity = fridgePortions`, `unit = "portions"`, `expiry_date = today + fridgeMaxDays`, default 4 days) and/or a freezer row (`quantity = freezerPortions`, full freezer extension: `frozen_at = today`, `max_freeze_weeks` default 12, `defrost_method = OVERNIGHT_FRIDGE`). Portion counts come from `command.batchSplit` (`BatchCookSplitDirective`); a null directive defaults the whole batch (`servingsCooked` portions) to the fridge. Both rows: `source = BATCH_COOK`, `source_recipe_id = recipeId`, `name = command.batchLabel` (or a recipe-derived default). The new portion ids ride the same coalesced `ItemQuantityAdjustedEvent`.
...
**Idempotency (locked 2026-05-07).** A duplicate cook event for the same `mealSlotId` is treated as the same cook, not a second one. Mechanism:
...
- `applyCookEvent` checks for an existing row before applying; if present, no-op and log INFO.
...
The grocery module's `OrderReconciler` calls `applyGroceryOrder` directly at reconcile time (not via `GroceryOrderConfirmedEvent` — see §Events Consumed). `@Transactional`.

0. **Gate.** When the user has `pantry_tracking_enabled = false` the import is a no-op returning an empty result (LLD §Pantry tracking disable line 139) — the order still happened, provisions just doesn't track stock.
1. **Idempotency.** A dedicated `provision_grocery_import_log` table keyed on `(userId, source, orderRef)` — if a row exists, `DuplicateGroceryImportException` (409). Replays must be no-ops.
...
### Flow 8: Expiry sweep — *moved to notification/01b*

**Reconciliation note (2026-05):** the scheduled expiry-scan + alert ownership moved out of provisions to the **notification** module (notification/01b's scanner subsystem). Provisions no longer runs a `@Scheduled` expiry sweep and no longer publishes `ItemNearingExpiryEvent`; the `ExpiryInferenceService` here is a rule-registry façade only (used at grocery-import time to infer an item's expiry date). Provisions exposes the **read helpers** the notification scanner calls — `getExpiringInventory(userId, maxExpiryDate)` and the cross-user `findAllExpiringBefore(cutoff)` repository query — but does not itself scan, threshold, or alert. The per-location threshold (default 2 days fridge, 14 days freezer) and the fan-out to user alerts live in notification.
...
| `ProvisionForPlannerServiceIT` | `getBundle` returns active inventory + staples-needing-replenishment + equipment + budget + price map in one round trip. Bundle staleness flags partial coverage and the 8-week ramp-up window correctly. Verifies ≤ 5 SQL statements via Hibernate statistics — the planner calls this once per planning run, must be cheap. Pantry-tracking gating: when `pantry_tracking_enabled = false` the bundle's inventory + staples lists are empty (equipment/budget/prices still flow). |
...
| ~~`ExpirySweepIT`~~ | *Relocated to the notification module (notification/01b).* The `@Scheduled` expiry scan + `ItemNearingExpiryEvent` publication moved out of provisions (see Flow 8); provisions only owns the read helpers it exposes. The expiring-read helpers are covered by `PlannerBundleFlowIT` / service tests. |
