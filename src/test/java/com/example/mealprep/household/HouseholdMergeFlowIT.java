...
/**
 * Full HTTP flow over {@code POST /api/v1/households/current/merge}. Default profile uses the
 * {@code NoopSoftPreferencesReader} (no other bean wins) → merge returns empty docs. The
...
class HouseholdMergeFlowIT {
  ...
  @Test
  void merge_byPrimary_nullEaters_returns200_withEmptyDocs_noopReader() throws Exception {
  ...
  @Test
  void merge_noopReaderBeanIsWired() {
    // Confirms the @ConditionalOnMissingBean(SoftPreferencesReader.class) fired: the autowired
    // bean is the Noop class.
    assertThat(softPreferencesReader.getClass().getName()).endsWith("$NoopSoftPreferencesReader");
  }
